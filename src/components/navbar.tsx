  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-transparent px-6 py-3 flex items-center justify-between">
      ...
      <div className="flex space-x-6 font-medium">
        {["Home", "Collections", "Sustainability", "Color Cards"].map((item) => (
          <Link
            key={item}
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2">
        <h1 className="text-2xl font-bold text-white">Runsun</h1>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex border border-white rounded-md overflow-hidden text-sm">
          <button
            ...
            className={`px-3 py-1 transition ${activeLang === "EN" ? "bg-orange-400 text-white" : "text-white bg-white"}`}
          >
            EN
          </button>
          <button
            onClick={() => setActiveLang("中文")}
            className={`px-3 py-1 transition ${activeLang === "中文" ? "bg-orange-400 text-white" : "text-white bg-white"}`}
          >
            中文
          </button>
        </div>
